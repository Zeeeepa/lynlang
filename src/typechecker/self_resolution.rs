use crate::ast::{AstType, Function, Statement, Expression, TraitImplementation};
use crate::error::Result;

/// Transform Self types to concrete types in trait implementations
pub fn transform_trait_impl_self_types(trait_impl: &TraitImplementation) -> Result<TraitImplementation> {
...
pub fn replace_self_in_ast_type(ast_type: &AstType, concrete_type: &str) -> AstType {
    match ast_type {
        ...
        AstType::DynVec { element_types, allocator_type } => {
            AstType::DynVec {
                element_types: element_types.iter()
                    .map(|t| replace_self_in_ast_type(t, concrete_type))
                    .collect(),
                allocator_type: allocator_type.as_ref()
...
fn transform_statements_self_types(stmts: &[Statement], concrete_type: &str) -> Result<Vec<Statement>> {
...
fn transform_statement_self_types(stmt: &Statement, concrete_type: &str) -> Result<Statement> {
    match stmt {
        Statement::VariableDeclaration { name, type_, declaration_type, initializer, is_mutable } => {
            Ok(Statement::VariableDeclaration {
                name: name.clone(),
                type_: type_.as_ref().map(|t| replace_self_in_ast_type(t, concrete_type)),
        ...
        Statement::Expression(expr) => {
            Ok(Statement::Expression(transform_expression_self_types(expr, concrete_type)?))
        }
        Statement::Return(expr) => {
            Ok(Statement::Return(transform_expression_self_types(expr, concrete_type)?))
        }
