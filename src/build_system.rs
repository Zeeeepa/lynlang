...
impl BuildContext {
    ...
    pub fn new(project_root: impl AsRef<Path>) -> Result<Self> {
        ...
        if !config_path.exists() {
            return Err(CompileError::BuildError(
                format!("No zen.toml found in {}", project_root.display())
            ));
        }
    ...
    pub fn resolve_dependencies(&mut self) -> Result<()> {
        let deps: Vec<(String, Dependency)> = self.config.dependencies
            .iter()
    ...
    fn resolve_dependency(&mut self, name: &str, dep: &Dependency) -> Result<()> {
        let resolved = if let Some(path) = &dep.path {
            ...
            if !dep_path.exists() {
                return Err(CompileError::BuildError(
                    format!("Local dependency '{}' not found at {}", name, dep_path.display())
                ));
            }
            ResolvedDependency {
                name: name.to_string(),
    ...
    fn fetch_git_dependency(&self, name: &str, url: &str, dep: &Dependency) -> Result<ResolvedDependency> {
        ...
        if !dep_dir.exists() {
            ...
            let output = cmd.output()
                .map_err(|e| CompileError::BuildError(format!("Failed to clone {}: {}", name, e)))?;
            
            if !output.status.success() {
                return Err(CompileError::BuildError(
                    format!("Failed to clone {}: {}", name, String::from_utf8_lossy(&output.stderr))
                ));
            }
            ...
            if let Some(branch) = &dep.branch {
                Command::new("git")
                    ...
                    .output()
                    .map_err(|e| CompileError::BuildError(format!("Failed to checkout branch: {}", e)))?;
            } else if let Some(tag) = &dep.tag {
                Command::new("git")
                    .current_dir(&dep_dir)
                    .args(&["checkout", &format!("tags/{}", tag)])
                    .output()
                    .map_err(|e| CompileError::BuildError(format!("Failed to checkout tag: {}", e)))?;
            }
        }
    ...
    fn fetch_registry_dependency(&self, name: &str, version: &str) -> Result<ResolvedDependency> {
        ...
        if !dep_dir.exists() {
            ...
            return Err(CompileError::BuildError(
                format!("Package '{}' version '{}' not found in registry", name, version)
            ));
        }
    ...
    fn run_build_script(&self, script: &str) -> Result<()> {
        ...
        if !script_path.exists() {
            return Err(CompileError::BuildError(
                format!("Build script '{}' not found", script)
            ));
        }
    ...
    fn collect_zen_files(&self, dir: &Path, files: &mut Vec<PathBuf>) -> Result<()> {
        ...
        {
            let entry = entry.map_err(|e| CompileError::BuildError(format!("Failed to read entry: {}", e)))?;
            let path = entry.path();
    ...
    fn build_dependency_graph(&mut self, files: &[PathBuf]) -> Result<()> {
        for file in files {
            ...
            let metadata = fs::metadata(file)
                .map_err(|e| CompileError::BuildError(format!("Failed to get file metadata: {}", e)))?;
    ...
    fn propagate_dirty_flag(&mut self) {
        ...
        while changed {
            changed = false;
            let dirty_files: Vec<String> = self.build_graph.nodes
                .iter()
    ...
    fn build_executable(&self, main: &str, _source_files: &[PathBuf]) -> Result<PathBuf> {
        ...
        if !main_path.exists() {
            return Err(CompileError::BuildError(
                format!("Main file '{}' not found", main)
            ));
        }
    ...
    fn build_library(&self, lib: &str, _source_files: &[PathBuf]) -> Result<PathBuf> {
        ...
        if !lib_path.exists() {
            return Err(CompileError::BuildError(
                format!("Library file '{}' not found", lib)
            ));
        }
    ...
    fn save_build_cache(&self) -> Result<()> {
        ...
        let cache: HashMap<String, u64> = self.build_graph.nodes
            .iter()
    ...
    pub fn clean(&self) -> Result<()> {
        if self.build_dir.exists() {
            fs::remove_dir_all(&self.build_dir)
                .map_err(|e| CompileError::BuildError(format!("Failed to clean build directory: {}", e)))?;
        }
        
        if self.cache_dir.exists() {
            fs::remove_dir_all(&self.cache_dir)
                .map_err(|e| CompileError::BuildError(format!("Failed to clean cache directory: {}", e)))?;
        }
