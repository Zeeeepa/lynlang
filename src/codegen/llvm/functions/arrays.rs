...
use inkwell::{module::Linkage, values::{BasicValueEnum, PointerValue}};
...
pub fn compile_array_new<'ctx>(
    ...
    args: &[ast::Expression],
        ...
        if remaining_args.is_empty() {
            ...
            let total_size = compiler.builder.build_int_mul(default_capacity, element_size, "total_size")?;
            ...
            let data_ptr = compiler.builder.build_call(malloc_fn, &[total_size.into()], "array_data")?
                .try_as_basic_value()
                .left()
            ...
            // Set data pointer
            let data_field_ptr = compiler.builder.build_struct_gep(array_struct_type, array_val, 0, "data_ptr")?;
            compiler.builder.build_store(data_field_ptr, data_ptr)?;
            
            // Set length to 0 (empty array)
            let length_field_ptr = compiler.builder.build_struct_gep(array_struct_type, array_val, 1, "length_ptr")?;
            compiler.builder.build_store(length_field_ptr, compiler.context.i64_type().const_int(0, false))?;
            
            // Set capacity to 10
            let capacity_field_ptr = compiler.builder.build_struct_gep(array_struct_type, array_val, 2, "capacity_ptr")?;
            compiler.builder.build_store(capacity_field_ptr, default_capacity)?;
            ...
            return Ok(compiler.builder.build_load(array_struct_type, array_val, "array_loaded")?.into());
        }
        ...
        let capacity = if capacity_raw.get_type() == compiler.context.i64_type() {
            ...
            compiler.builder.build_int_z_extend(capacity_raw, compiler.context.i64_type(), "capacity_i64")?
        };
        ...
        // Store data pointer
        let data_ptr_field = compiler.builder.build_struct_gep(
            array_struct_type,
            array_alloca,
            0,
            "data_ptr_field",
        ...
        let result = compiler.builder.build_load(array_struct_type, array_alloca, "array_value")?;
        Ok(result)
    }
...
pub fn compile_array_push_by_ptr<'ctx>(
        ...
        let current_length = compiler.builder.build_load(compiler.context.i64_type(), length_ptr, "current_length")?
            .into_int_value();
        ...
        let data_ptr = compiler.builder.build_load(
        ...
        let value_to_store = if value.is_pointer_value() {
            ...
            let ptr = compiler.builder.build_call(malloc_fn, &[alloc_size.into()], "int_ptr")?
                .try_as_basic_value()
                .left()
            ...
            let value_i64 = if int_val.get_type() == compiler.context.i64_type() {
                ...
                compiler.builder.build_int_s_extend(int_val, compiler.context.i64_type(), "value_i64")?
            };
            ...
            let ptr = compiler.builder.build_call(malloc_fn, &[alloc_size.into()], "float_ptr")?
                .try_as_basic_value()
                .left()
                .ok_or_else(|| CompileError::InternalError("malloc failed".to_string(), None))?
                .into_pointer_value();
            ...
            let ptr = compiler.builder.build_call(malloc_fn, &[alloc_size.into()], "struct_ptr")?
                .try_as_basic_value()
                .left()
                .ok_or_else(|| CompileError::InternalError("malloc failed".to_string(), None))?
                .into_pointer_value();
            ...
            let ptr = compiler.builder.build_call(malloc_fn, &[alloc_size.into()], "value_ptr")?
                .try_as_basic_value()
                .left()
                .ok_or_else(|| CompileError::InternalError("malloc failed".to_string(), None))?
                .into_pointer_value();
...
pub fn compile_array_push<'ctx>(
        ...
        let array_ptr = compiler.builder.build_alloca(array_struct_type, "array_ptr")?;
        ...
        // Get capacity
        let capacity_ptr = compiler.builder.build_struct_gep(
            array_struct_type,
            array_ptr,
            2,
            "capacity_ptr",
        )?;
        let _capacity = compiler.builder.build_load(compiler.context.i64_type(), capacity_ptr, "capacity")?
            .into_int_value();
...
pub fn compile_array_get<'ctx>(
        ...
        // Store array to get a pointer
        let array_ptr = compiler.builder.build_alloca(array_struct_type, &format!("array_get_ptr_{}", unique_id))?;
        compiler.builder.build_store(array_ptr, array_val)?;
        ...
        let index_i64 = if index.get_type() == compiler.context.i64_type() {
            ...
            compiler.builder.build_int_s_extend(index, compiler.context.i64_type(), &format!("get_index_i64_{}", unique_id))?
        };
        ...
        let in_bounds_bb = compiler.context.append_basic_block(current_fn, &format!("get_in_bounds_{}", unique_id));
        let out_bounds_bb = compiler.context.append_basic_block(current_fn, &format!("get_out_bounds_{}", unique_id));
        let merge_bb = compiler.context.append_basic_block(current_fn, &format!("get_merge_{}", unique_id));
        
        compiler.builder.build_conditional_branch(in_bounds, in_bounds_bb, out_bounds_bb)?;
        ...
        let value_ptr = compiler.builder.build_load(ptr_type, element_ptr, &format!("get_elem_ptr_val_{}", unique_id))?.into_pointer_value();
        
        // Load the value as i64 (since push stores as i64) then truncate to i32
        let value_i64 = compiler.builder.build_load(compiler.context.i64_type(), value_ptr, &format!("get_elem_val_i64_{}", unique_id))?.into_int_value();
        let value = compiler.builder.build_int_truncate(value_i64, compiler.context.i32_type(), &format!("get_elem_val_{}", unique_id))?;
        
        // Create Some(value) - allocate and build struct
        let some_alloca = compiler.builder.build_alloca(option_type, &format!("some_alloca_{}", unique_id))?;
        let disc_ptr = compiler.builder.build_struct_gep(option_type, some_alloca, 0, &format!("some_disc_ptr_{}", unique_id))?;
        let payload_ptr = compiler.builder.build_struct_gep(option_type, some_alloca, 1, &format!("some_payload_ptr_{}", unique_id))?;
        
        compiler.builder.build_store(disc_ptr, compiler.context.i64_type().const_int(0, false))?;  // 0 for Some
        compiler.builder.build_store(payload_ptr, value)?;
        
        let some_val = compiler.builder.build_load(option_type, some_alloca, &format!("some_val_{}", unique_id))?;
        ...
        let none_alloca = compiler.builder.build_alloca(option_type, &format!("none_alloca_{}", unique_id))?;
        let disc_ptr = compiler.builder.build_struct_gep(option_type, none_alloca, 0, &format!("none_disc_ptr_{}", unique_id))?;
        let payload_ptr = compiler.builder.build_struct_gep(option_type, none_alloca, 1, &format!("none_payload_ptr_{}", unique_id))?;
        
        compiler.builder.build_store(disc_ptr, compiler.context.i64_type().const_int(1, false))?;  // 1 for None
        compiler.builder.build_store(payload_ptr, compiler.context.i32_type().const_int(0, false))?;  // dummy payload
        
        let none_val = compiler.builder.build_load(option_type, none_alloca, &format!("none_val_{}", unique_id))?;
        ...
        let phi = compiler.builder.build_phi(option_type, &format!("get_result_{}", unique_id))?;
...
pub fn compile_array_len<'ctx>(
        ...
        // Store array to get a pointer
        let array_ptr = compiler.builder.build_alloca(array_struct_type, &format!("array_len_ptr_{}", unique_id))?;
        compiler.builder.build_store(array_ptr, array_val)?;
...
pub fn compile_array_set<'ctx>(
        ...
        // Store array to get a pointer
        let array_ptr = compiler.builder.build_alloca(array_struct_type, &format!("array_set_ptr_{}", unique_id))?;
        compiler.builder.build_store(array_ptr, array_val)?;
        ...
        let index_i64 = if index.get_type() == compiler.context.i64_type() {
            ...
            compiler.builder.build_int_s_extend(index, compiler.context.i64_type(), &format!("set_index_i64_{}", unique_id))?
        };
        ...
        let value_to_store = if value.is_int_value() {
            ...
            if int_val.get_type() == compiler.context.i64_type() {
                ...
                compiler.builder.build_int_s_extend(int_val, compiler.context.i64_type(), &format!("set_value_i64_{}", unique_id))?
            }
        } else {
            return Err(CompileError::TypeError(
...
pub fn compile_array_pop_by_ptr<'ctx>(
        ...
        let value_ptr = compiler.builder.build_alloca(compiler.context.i32_type(), &format!("pop_return_val_{}", unique_id))?;
        ...
        let current_fn = compiler.builder.get_insert_block().unwrap().get_parent().unwrap();
        let empty_bb = compiler.context.append_basic_block(current_fn, &format!("pop_empty_{}", unique_id));
        let nonempty_bb = compiler.context.append_basic_block(current_fn, &format!("pop_nonempty_{}", unique_id));
        let merge_bb = compiler.context.append_basic_block(current_fn, &format!("pop_merge_{}", unique_id));
        ...
        compiler.builder.build_store(value_ptr, compiler.context.i32_type().const_zero())?;
        
        let none_val = {
            let discriminant = compiler.context.i64_type().const_int(1, false); // 1 for None (matching registration)
            let null_ptr = compiler.context.ptr_type(inkwell::AddressSpace::default()).const_null();
        ...
        let new_length = compiler.builder.build_int_sub(length, one, &format!("pop_new_len_{}", unique_id))?;
        ...
        let value_i64 = compiler.builder.build_load(compiler.context.i64_type(), element_ptr, &format!("pop_val_i64_{}", unique_id))?;
        ...
        let some_alloca = compiler.builder.build_alloca(option_type, &format!("pop_some_alloca_{}", unique_id))?;
        
        // Store discriminant - Some is 0 (matching registration)
        let tag_ptr = compiler.builder.build_struct_gep(option_type, some_alloca, 0, &format!("pop_tag_ptr_{}", unique_id))?;
        compiler.builder.build_store(tag_ptr, compiler.context.i64_type().const_zero())?;
        
        // Store payload pointer
        let payload_ptr = compiler.builder.build_struct_gep(option_type, some_alloca, 1, &format!("pop_payload_ptr_{}", unique_id))?;
        compiler.builder.build_store(payload_ptr, value_ptr)?;
        
        // Load the struct
        let some_val = compiler.builder.build_load(option_type, some_alloca, &format!("pop_some_val_{}", unique_id))?;
        compiler.builder.build_unconditional_branch(merge_bb)?;
        
        // Merge block
        compiler.builder.position_at_end(merge_bb);
        let phi = compiler.builder.build_phi(option_type, &format!("pop_result_{}", unique_id))?;
...
pub fn compile_array_pop<'ctx>(
        ...
        // Store array to get a pointer
        let array_ptr = compiler.builder.build_alloca(array_struct_type, "array_pop_temp")?;
        compiler.builder.build_store(array_ptr, array_val)?;
