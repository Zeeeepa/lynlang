    ...
    pub fn get_or_create_runtime_function(
        ...
        match name {
            ...
            "string_trim" => {
                ...
                let start_block = self.context.append_basic_block(func, "find_start");
                let start_loop_block = self.context.append_basic_block(func, "start_loop");
                let start_check_block = self.context.append_basic_block(func, "start_check");
                let start_found_block = self.context.append_basic_block(func, "start_found");
                
                // Initialize start index
                let start_ptr = self.builder.build_alloca(self.context.i64_type(), "start_idx")?;
                self.builder.build_store(start_ptr, self.context.i64_type().const_int(0, false))?;
                self.builder.build_unconditional_branch(start_block)?;
                
                // Start loop - find first non-whitespace
                self.builder.position_at_end(start_block);
                self.builder.build_unconditional_branch(start_loop_block)?;
                
                self.builder.position_at_end(start_loop_block);
                let start_idx = self.builder.build_load(self.context.i64_type(), start_ptr, "start_val")?.into_int_value();
                let at_end = self.builder.build_int_compare(
                    inkwell::IntPredicate::UGE,
                    start_idx,
                    len,
                    "at_end"
                )?;
                self.builder.build_conditional_branch(at_end, start_found_block, start_check_block)?;
                
                // Check if current char is whitespace
                self.builder.position_at_end(start_check_block);
                let char_ptr = unsafe {
                    self.builder.build_in_bounds_gep(
                        self.context.i8_type(),
                        str_param,
                ...
                let char_val = self.builder.build_load(self.context.i8_type(), char_ptr, "char")?.into_int_value();
                ...
                let is_ws1 = self.builder.build_or(is_space, is_tab, "ws1")?;
                let is_ws2 = self.builder.build_or(is_newline, is_cr, "ws2")?;
                let is_whitespace = self.builder.build_or(is_ws1, is_ws2, "is_whitespace")?;
                
                let inc_block = self.context.append_basic_block(func, "inc_start");
                self.builder.build_conditional_branch(is_whitespace, inc_block, start_found_block)?;
                ...
                self.builder.build_store(start_ptr, next_idx)?;
                self.builder.build_unconditional_branch(start_loop_block)?;
                
                // Find end position (skip trailing whitespace)
                self.builder.position_at_end(start_found_block);
                let final_start = self.builder.build_load(self.context.i64_type(), start_ptr, "final_start")?.into_int_value();
                
                // If entire string was whitespace, return empty string
                let all_whitespace = self.builder.build_int_compare(
                    inkwell::IntPredicate::UGE,
                    final_start,
                    len,
                    "all_whitespace"
                ...
                let empty_block = self.context.append_basic_block(func, "empty_string");
                let find_end_block = self.context.append_basic_block(func, "find_end");
                self.builder.build_conditional_branch(all_whitespace, empty_block, find_end_block)?;
                
                // Return empty string
                self.builder.position_at_end(empty_block);
                let empty_size = self.context.i64_type().const_int(1, false);
                let empty_ptr = self.builder.build_call(
                    malloc_fn,
                    &[empty_size.into()],
                    "empty_ptr"
                )?.try_as_basic_value().left().unwrap().into_pointer_value();
                self.builder.build_store(empty_ptr, self.context.i8_type().const_int(0, false))?;
                self.builder.build_return(Some(&empty_ptr))?;
                
                // Find end position
                self.builder.position_at_end(find_end_block);
                let end_ptr = self.builder.build_alloca(self.context.i64_type(), "end_idx")?;
                let len_minus_1 = self.builder.build_int_sub(
                    len,
                    self.context.i64_type().const_int(1, false),
                    "len_minus_1"
                )?;
                self.builder.build_store(end_ptr, len_minus_1)?;
                
                let end_loop_block = self.context.append_basic_block(func, "end_loop");
                let end_check_block = self.context.append_basic_block(func, "end_check");
                let end_found_block = self.context.append_basic_block(func, "end_found");
                self.builder.build_unconditional_branch(end_loop_block)?;
                
                // End loop - find last non-whitespace
                self.builder.position_at_end(end_loop_block);
                let end_idx = self.builder.build_load(self.context.i64_type(), end_ptr, "end_val")?.into_int_value();
                let at_start = self.builder.build_int_compare(
                    inkwell::IntPredicate::ULE,
                    end_idx,
                    final_start,
                    "at_start"
                )?;
                self.builder.build_conditional_branch(at_start, end_found_block, end_check_block)?;
                ...
                let end_char = self.builder.build_load(self.context.i8_type(), end_char_ptr, "end_char")?.into_int_value();
                
                // Check for whitespace characters
                let end_is_space = self.builder.build_int_compare(
                    inkwell::IntPredicate::EQ,
                    end_char,
                    self.context.i8_type().const_int(32, false),
                    "end_is_space"
                )?;
                let end_is_tab = self.builder.build_int_compare(
                    inkwell::IntPredicate::EQ,
                    end_char,
                    self.context.i8_type().const_int(9, false),
                    "end_is_tab"
                )?;
                let end_is_newline = self.builder.build_int_compare(
                    inkwell::IntPredicate::EQ,
                    end_char,
                    self.context.i8_type().const_int(10, false),
                    "end_is_newline"
                )?;
                let end_is_cr = self.builder.build_int_compare(
                    inkwell::IntPredicate::EQ,
                    end_char,
                    self.context.i8_type().const_int(13, false),
                    "end_is_cr"
                ...
                let end_ws1 = self.builder.build_or(end_is_space, end_is_tab, "end_ws1")?;
                let end_ws2 = self.builder.build_or(end_is_newline, end_is_cr, "end_ws2")?;
                let end_is_whitespace = self.builder.build_or(end_ws1, end_ws2, "end_is_whitespace")?;
                
                let dec_block = self.context.append_basic_block(func, "dec_end");
                self.builder.build_conditional_branch(end_is_whitespace, dec_block, end_found_block)?;
                
                // Decrement end index
                self.builder.position_at_end(dec_block);
                let prev_idx = self.builder.build_int_sub(
                    end_idx,
                    self.context.i64_type().const_int(1, false),
                    "prev_end"
                )?;
                self.builder.build_store(end_ptr, prev_idx)?;
                self.builder.build_unconditional_branch(end_loop_block)?;
                
                // Create trimmed string
                self.builder.position_at_end(end_found_block);
                let final_end = self.builder.build_load(self.context.i64_type(), end_ptr, "final_end")?.into_int_value();
                
                // Calculate trimmed length (end - start + 1)
                let trimmed_len = self.builder.build_int_sub(final_end, final_start, "diff")?;
                ...
                let _ = self.builder.build_call(
                    memcpy_fn,
                    &[result_ptr.into(), src_ptr.into(), trimmed_len.into()],
                    "memcpy_result"
                )?;
