...
pub fn get_code_actions(
    ...
    for diagnostic in &context.diagnostics {
        ...
        if diagnostic
            .message
            .contains("Import statements must be at module level")
        {
        ...
        if diagnostic.message.contains("'if' keyword")
            || diagnostic.message.contains("'else' keyword")
        {
            actions.push(CodeActionOrCommand::CodeAction(CodeAction {
                                ...
                                || after_name.trim_start().starts_with('<')
                            {
                                let symbol = Symbol {
                                    ...
                                    range: Range::new(
                                        ...
                                        Position::new(
                                            line_idx as u32 + func.body.len() as u32 + 1,
                                            0,
                                        ),
                                    ),
                                    detail: Some(format!(
                    ...
                    return Some(Location {
                        ...
                        range: Range::new(
                            ...
                            Position::new(
                                ...
                                (param_pos + param_name.len()) as u32,
                            ),
                        ),
                    });
                }
    ...
    if identifier.starts_with("@std") {
        ...
        return Some(Hover {
            ...
            range: Some(Range::new(
                Position::new(
                    position.line,
                    position.character.saturating_sub(identifier.len() as u32),
                ),
    ...
    if let Some(symbol) = table.find_symbol(&identifier) {
        ...
        match symbol.kind {
            ...
            SymbolKind::STRUCT => {
                ...
                hover_text.push_str("\n💡 **Usage:**\n");
                hover_text.push_str(&format!(
                    "- Constructor: `{} {{ field1: value1, field2: value2 }}`\n",
                    identifier
                ));
                hover_text.push_str(&format!(
                    "- UFCS methods: `value.method()` or `{}::method(value)`\n",
                    identifier
                ));
                    ...
                    if !type_name.is_empty() && !type_name.starts_with(':') {
                        ...
                        return Some(Hover {
                            ...
                            range: Some(Range::new(
                                ...
                                Position::new(
                                    position.line,
                                    position.character + identifier.len() as u32,
                                ),
                            )),
                        });
                    }
...
pub fn get_std_module_info(module: &str) -> String {
    match module {
        "@std.io" => "**@std.io** - Input/Output Module\n\n\
            Functions:\n\
            ...
            .to_string(),
        "@std.mem" => "**@std.mem** - Memory Management\n\n\
            Types:\n\
            ...
            - `free<T>(ptr: Ptr<T>)`"
            .to_string(),
        "@std.math" => "**@std.math** - Mathematics\n\n\
            Constants:\n\
            - `PI`, `E`, `TAU`\n\
            Functions:\n\
            - `sin`, `cos`, `tan`, `sqrt`, `pow`, `abs`, `min`, `max`"
            .to_string(),
