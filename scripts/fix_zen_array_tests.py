...
def fix_array_test(filepath):
    ...
    # Fix imports for zen_test files (might have different patterns)
    content = re.sub(
        r'\{ ([^}]*)\bArray\b([^}]*) \}',
        lambda m: '{ ' + m.group(1).replace('Array', '').strip(', ') + (', ' if m.group(1).strip() else '') + 'get_default_allocator' + m.group(2) + ' }',
        content
    )
...
def main():
    ...
    for filename in sorted(os.listdir(test_dir)):
        if filename.startswith('zen_test_array') and filename.endswith('.zen'):
